...
import {
  getUserHierarchy,
  getUserWarehouseHierarchy,
} from "../rolePermission/permission.js";
import { Warehouse } from "../model/warehouse.model.js";

export const UserXml = async (req, res) => {
  const fileUrl =
    "https://xmlfile.blr1.cdn.digitaloceanspaces.com/Createuser.xml";
  try {
...
export const SaveUser = async (req, res, next) => {
  try {
    ...
    return user
      ? res
          .status(200)
          .json({ message: "Data Save Successfully", User: user, status: true })
      : res
          .status(400)
...
export const SignIn = async (req, res, next) => {
  try {
    ...
    if (!existingAccount) {
      existingAccount = await Warehouse.findOne({ email }).populate({
        path: "rolename",
        model: "role",
      });
      if (!existingAccount) {
      ...
      transporter.sendMail(mailOptions, (error, info) => {
        if (!error) {
          return res
            .status(201)
            .json({
              user: { ...existingAccount.toObject(), password: undefined },
          ...
          return res
            .status(500)
            .json({ error: "Failed to send OTP email", status: false });
        }
      });
    } else {
