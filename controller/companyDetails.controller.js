...
import {
  getCompanyDetailHierarchy,
  getCompanyDetailHierarchy1,
...
export const viewCompanyDetails = async (req, res, next) => {
  try {
    ...
    const adminDetails = await getCompanyDetailHierarchy1(userId);
    const adminDetailss =
      adminDetails.length === 1 ? adminDetails[0] : adminDetails;
    const adminDetail = adminDetailss[adminDetailss.length - 1];
    const companyDetail = await CompanyDetails.find().sort({ sortorder: -1 });
    return companyDetail
      ? res.status(200).json({ CompanyDetail: adminDetail, status: true })
